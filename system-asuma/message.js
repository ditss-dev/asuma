...
async function GroupUpdate(Ditss, m, store) {
	if (!m.messageStubType || !m.isGroup) return
	if (global.db?.groups?.[m.chat]?.setinfo && Ditss.public) {
		...
		if (messages[m.messageStubType]) {
			await Ditss.sendMessage(m.chat, { text: `${admin} ${messages[m.messageStubType]}`, mentions: [m.sender, ...(m.messageStubParameters[0]?.includes('@') ? [`${m.messageStubParameters[0]}`] : [])]}, { ephemeralExpiration: m.expiration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		} else {
			console.log({
...
async function GroupParticipantsUpdate(Ditss, { id, participants, author, action }, store, groupCache) {
	try {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await Ditss.profilePictureUrl(n, 'image');
				} catch {
...
async function MessagesUpsert(Ditss, message, store, groupCache) {
	try {
		...
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) store.groupMetadata ??= await Ditss.groupFetchAllParticipating().catch(e => ({}));
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		const m = await Serialize(Ditss, msg, store, groupCache)
		require('../asuma')(Ditss, m, msg, store, groupCache);
		if (db?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await Ditss.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await Ditss.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await Ditss.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		throw e;
...
async function Solving(Ditss, store) {
	...
	Ditss.getName = (jid, withoutContact  = false) => {
		...
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || (store.groupMetadata[id] ? store.groupMetadata[id] : (store.groupMetadata[id] = Ditss.groupMetadata(id))) || {};
	...
	Ditss.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		...
		Ditss.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	...
	Ditss.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return Ditss.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	}
	...
	Ditss.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return Ditss.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, quotedOptions);
			} else if (mime && mime === 'application/pdf') {
				return Ditss.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('image')) {
				return Ditss.sendMessage(jid, { image: res.data, caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('video')) {
				return Ditss.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, quotedOptions);
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return Ditss.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return Ditss.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, quotedOptions);
			}
		}
	...
	Ditss.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await Ditss.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	
	Ditss.sendText = async (jid, text, quoted, options = {}) => Ditss.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	...
	Ditss.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return Ditss.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	...
	Ditss.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await Ditss.downloadMediaMessage(message);
	...
	Ditss.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: Ditss.user.id, quoted: m.quoted });
		apb.key = m.key
		apb.key.fromMe = areJidsSameUser(m.sender, Ditss.user.id);
		if (m.isGroup) apb.participant = m.sender;
		Ditss.ev.emit('messages.upsert', {
			...m,
	...
	Ditss.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		...
		let anu = await Ditss.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0, ...options });
	...
	Ditss.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await Ditss.relayMessage(msg.key.remoteJid, msg.message, {
	...
	Ditss.sendBettonImage = async (jid, content = {}, options = {}) => {
	const { text, caption, footer = '', image, buttons = [], mentions = [], contextInfo = {}, ai } = content;

	const message = {
		templateMessage: {
			hydratedTemplate: {
				imageMessage: (await generateWAMessageContent({ image }, { upload: Ditss.waUploadToServer })).imageMessage,
				hydratedContentText: text || caption || '',
				hydratedFooterText: footer,
				hydratedButtons: buttons.map(btn => {
					if (btn.urlButton) return { urlButton: btn.urlButton };
					if (btn.callButton) return { callButton: btn.callButton };
					if (btn.quickReplyButton) return { quickReplyButton: btn.quickReplyButton };
					return {};
				}),
				contextInfo: {
					...contextInfo,
	...
	Ditss.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: Ditss.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await Ditss.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (Ditss.user && Ditss.user.id) {
		const botNumber = Ditss.decodeJid(Ditss.user.id);
		if (global.db?.set[botNumber]) {
			Ditss.public = global.db.set[botNumber].public
		} else Ditss.public = true
	} else Ditss.public = true
...
async function Serialize(Ditss, msg, store, groupCache) {
	const botLid = Ditss.decodeJid(Ditss.user.lid);
	const botNumber = Ditss.decodeJid(Ditss.user.id);
	const m = { ...msg };
	if (!m) return m
	if (m.key) {
		...
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await Ditss.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = groupCache.get(m.chat))
			if (!metadata) {
				metadata = await Ditss.groupMetadata(m.chat).catch(e => ({}))
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.getQuotedObj = async () => {
				...
				return await Serialize(Ditss, q, store, groupCache)
			}
			m.quoted.key = {
	...
	m.download = () => Ditss.downloadMediaMessage(m)
